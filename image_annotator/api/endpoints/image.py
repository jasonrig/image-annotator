...
class AnnotateImage(Resource):
    ...
    def post(self, id, userid):
        ...
        result = db.add_bounding_box_annotation(id, userid,
                                                req_data['x_offset'],
                                                req_data['y_offset'],
                                                req_data['height'],
                                                req_data['width'])
...
class AddImage(Resource):
    ...
    def post(self, userid):
        ...
        if os.path.isfile(req_data['path']):
            if 'mimetype' not in req_data:
                mime = magic.Magic(mime=True)
                req_data['mimetype'] = mime.from_file(req_data['path'])
            return {
                'id': db.insert_image(req_data['path'], req_data['mimetype'])}
        else:
            abort(500)
